...
// 点击展开折叠折叠菜单
const foldBtn = document.querySelector('.burger')
const foldNav = document.querySelector('nav')
const navs = foldNav.querySelectorAll('a')
foldBtn &&
  foldBtn.addEventListener('click', () => {
    if (!headerEl.classList.contains('flod-show')) {
      // 禁止页面鼠标滚动和滑动
      document.body.style.overflowY = 'hidden'
      headerEl.classList.add('flod-show')

      // 监听滚动条滚动（点击锚点标签），如果发生滚动隐藏折叠导航栏
    } else {
      // 恢复页面鼠标滚动和滑动
      document.body.style.overflowY = 'auto'
      headerEl.classList.remove('flod-show')
    }
  })
